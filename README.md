...
基于 **Python + PyQt6 + OpenCV + Ultralytics(YOLO)**，打包成免安装的便携版（绿色软件，拷走即用）。

## ⬇️ 下载即用（推荐，免安装）

**[📦 下载 v2.1.0 便携版（约 248 MB）](https://github.com/MZJ123-art/dataset-annotation-tool/releases/latest)**

1. 下载 zip → 解压到任意目录
2. 双击 `数据集标注工具.exe` 即可运行

- **不需要**安装 Python、**不需要**装依赖、**不需要**联网
- 拷到任何 Windows 10/11 电脑都能直接用；卸载 = 删除文件夹（不写注册表）

> - 首次运行若出现「Windows 已保护你的电脑」，点 **更多信息 → 仍要运行**（程序未购买数字签名证书，不是病毒）
> - 压缩包内**不含模型权重**：使用 AI 预标注时请自备 `.pt` / `.onnx` 模型，其它功能无需模型
> - 想自己从源码构建，见下方「打包成便携版」
